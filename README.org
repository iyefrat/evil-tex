...
** Misc.
The binding =M-n= is provided for =cd-latex= like brace movement
...
=evil-tex= does not require, but supports =evil-surround=. Many of the text
objects are defined by functions more complex than regexps, so if you have
=evil-embrace= installed as well, it will still use =evil-surround= for surround
bindings.

=evil-tex= also supports =which-key=, and if you are using =ts*= style toggles,
it plays nice with =evil-snipe= if installed
* Configuration
