...
- after doing =cse= you would will be able to enter one letter aliases to
  ...
  =cseE= for changing to =equation*=. One mapping, e.g. =csez=, will be reserved
  for entering a prompt so you can fully type out the thing.
...
- for commands maybe go straight to the prompt?
